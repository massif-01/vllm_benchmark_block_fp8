...
# Qwen3 Series Model Tuning Script
# Qwen3 系列模型调优脚本
# Usage: bash scripts/tune_qwen3.sh [model_name] [tp_size] [block_n] [block_k]
# 用法: bash scripts/tune_qwen3.sh [模型名称] [张量并行大小] [块大小N] [块大小K]
...
TP=${2:-4}                                   # Default TP size (recommended: 4-8 for 30B model) / 默认 TP 大小（30B 模型推荐：4-8）
BLOCK_N=${3:-128}                            # Default block_n / 默认 block_n
BLOCK_K=${4:-128}                            # Default block_k / 默认 block_k

echo "==================== Qwen3 Block FP8 Tuning ===================="
echo "Model: $MODEL"
...
SAVE_PATH=${SAVE_PATH:-"./tuned_configs/qwen3"}
